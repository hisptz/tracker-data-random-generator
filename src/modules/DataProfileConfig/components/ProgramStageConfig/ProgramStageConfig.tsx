...
export function ProgramStageConfig({
	programStage,
	name,
}: ProgramStageConfigProps) {
		...
		<div className="column gap-16">
			...
			>
				<div className="column gap-16 ph-16">
					...
					{repeatable && (
						<RHFTextInputField
							label={i18n.t("Number of events to be generated")}
							name={`${name}.count`}
							min={0}
							max={10}
							type="number"
				...
				<div className="column ph-16">
					...
					<div className="column gap-16">
						{dataElements.map((dataItem, fieldIndex) => (
							<DataItemConfigField
								key={`${dataItem.id}-config`}
								required={dataItem.compulsory}
								name={`${name}.dataElements.${fieldIndex}`}
								dataItem={dataItem}
							/>
						))}
					</div>
