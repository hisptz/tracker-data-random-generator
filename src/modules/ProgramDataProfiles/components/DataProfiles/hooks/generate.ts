...
import {ExportType, GenerateConfig} from "../components/GenerateDataModal";
...
export function useExportData() {
    const exportData = useCallback(
        async (data: any, types: ExportType[]) => {
...
export function useGenerateData(profileId: string | null, onComplete: () => void) {
    ...
    const generate = useCallback(async ({orgUnits, enrollmentTimeBoundary, count, exportTypes}: GenerateConfig) => {
        if (!profile) {
            return;
        }
        const meta = {
        ...
        await exportData(data, exportTypes);
        onComplete();
    }, [profile, program]);
