...
function generateAutogeneratedValuesQuery(
	values: DataItemConfig[],
	count: number,
	orgUnitCode: string,
) {
	return fromPairs(
		values.map(({ dataItemId, options }) => [
			dataItemId,
			{
...
export class EventRandomDataEngine {
	config: EventProgramDataConfiguration;
	program: Program;
	...
	constructor({
		...
		this.stage = head(this.program.programStages) as ProgramStage;

		this.dataElements =
			this.stage.programStageDataElements?.map(
	...
	async generateEventData({ count }: GenerateProps): Promise<Event[]> {
		const events: Event[] = [];
		await new Promise((resolve, reject) => {
			try {
				times(count, (index) => events.push(this.generateEvent()));
				resolve(null);
			} catch (e) {
				reject(e);
			}
	...
	generateEvent() {
		...
		const orgUnit = faker.helpers.arrayElement(
			this.config.meta.orgUnits?.map(({ id }) => id) as string[],
		);

		const { eventTimeBoundary } = meta ?? {};

		const minDate = new Date(eventTimeBoundary.min ?? "");
		const maxDate = new Date(eventTimeBoundary.max ?? "");

		const eventDate = faker.date
			.between({
				from: minDate,
				to: maxDate,
			})
		...
		const dataValues = this.config.dataElements?.map((config) =>
			this.generateDataItem(config),
		);
	...
	async generate(config: GenerateProps): Promise<Event[]> {
		return this.generateEventData(config);
	}

	generateDataItem(config: DataItemConfig): {
		dataElement: string;
		value: string;
	} {
		const { dataItemId, dataTypeName, options } = config ?? {};
		const dataGenerationConfig = find(supportedDataTypes, [
			"name",
			dataTypeName,
		]);
...
export class TrackerRandomDataEngine {
	...
	constructor({
		...
		this.program = program;
		this.attributes =
			this.program.programTrackedEntityAttributes?.map(
				({ trackedEntityAttribute }) => trackedEntityAttribute,
			) ?? [];
		this.dataElements =
			compact(
				flatten(
					this.program.programStages?.map(
	...
	async generateWithPagination({
		pages,
		pageSize,
	}: GenerateWithPaginationProps): Promise<TrackedEntityInstance[][]> {
		const teiPages: TrackedEntityInstance[][] = [];
		times(pages, async () =>
			teiPages.push(await this.generate({ count: pageSize ?? 50 })),
		);
		return teiPages;
	}

	async getAutogeneratedValues(count: number) {
		const autogeneratedAttributes = this.config.attributes?.filter(
			({ dataTypeName }) => {
		...
		const query = generateAutogeneratedValuesQuery(
	...
	async generateTrackerData({
		count,
	}: GenerateProps): Promise<TrackedEntityInstance[]> {
	...
	async generate(params: GenerateProps): Promise<TrackedEntityInstance[]> {
		return this.generateTrackerData(params);
	}
	...
	generateDataItem<T>(
		...
		if (
			dataGenerationConfig?.name === SupportedDataTypeNames.AUTOGENERATED
		) {
	...
	generateTrackerEvent({
		...
		const minDate =
			new Date(eventTimeBoundary.min ?? "") <
			new Date(enrollmentDate ?? "")
				? new Date(enrollmentDate ?? "")
				: new Date(eventTimeBoundary.min ?? "");
		const maxDate =
			new Date(eventTimeBoundary.max ?? "") >
			new Date(enrollmentDate ?? "")
				? new Date(eventTimeBoundary.max ?? "")
				: new Date(enrollmentDate ?? "");
		const eventDate = faker.date
			.between(minDate ?? new Date(), maxDate ?? new Date())
			.toISOString();
		const status = "COMPLETED";

		const dataValues = dataElementsConfig?.map((config) =>
	...
	generateTei(teiIndex: number): TrackedEntityInstance {
		...
		const enrollmentDate = faker.date
			.between(
				new Date(this.config.meta.enrollmentTimeBoundary?.min ?? "") ??
					new Date(),
				new Date(this.config.meta.enrollmentTimeBoundary?.max ?? "") ??
					new Date(),
			)
			.toISOString();
		const orgUnit = faker.helpers.arrayElement(
		...
		const trackedEntityAttributes = this.config.attributes?.map(
			(attributeConfig) =>
				this.generateDataItem<{
					attribute: string;
					value: any;
		...
		const events = flattenDeep(
			this.config.programStages?.map(
				({ id, eventTimeBoundary, dataElements, count }) => {
					const programStage = find(this.program.programStages, [
						"id",
						id,
					]);
					...
					return this.generateTrackerEvent({
						enrollmentDate,
						orgUnit,
						programStage: id,
						enrollment,
